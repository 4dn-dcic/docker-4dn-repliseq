...
This repo contains the source files for a docker image stored in duplexa/4dn-repliseq:v8. (we will change the docker hub account soon)
...
You need docker daemon to rebuild the docker image. If you want to push it to a different docker repo, replace duplexa/4dn-repliseq:v8 with your desired docker repo name. You need permission to push to duplexa/4dn-repliseq:v8.
```
docker build -t duplexa/4dn-repliseq:v8 .
docker push duplexa/4dn-repliseq:v8
```
You can skip this if you want to use an already built image on docker hub (image name duplexa/4dn-repliseq:v8). The command 'docker run' (below) automatically pulls the image from docker hub.
...
```
docker run duplexa/4dn-repliseq:v8 /usr/bin/time du 2> log
cat log
```
...
```
# default
docker run duplexa/4dn-repliseq:v8

# specific run command
docker run duplexa/4dn-repliseq:v8 <run-xx.sh> <arg1> <arg2> ...

# may need -v option to mount data file/folder if they are used as arguments.
docker run -v /data1/:/d1/:rw -v /data2/:/d2/:rw duplexa/4dn-repliseq:v8 <run-xx.sh> /d1/file1 /d2/file2 ...
```
